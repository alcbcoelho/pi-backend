...
router.get("/", (req, res, next) => {
  ...
  if (req.query.name && req.query.artist) {
    if (songByName.length && songByArtist.length) {
      ...
      songByName.length && res.status(404).send(errorMessage.artists[0]);
      songByArtist.length && res.status(404).send(errorMessage.songs[0]);
      res
        .status(404)
        .send(`${errorMessage.artists[0]}<br><br>${errorMessage.songs[0]}`);
    }
  } else if (req.query.name) {
    if (songByName.length) {
      songByName.length === 1 && res.status(200).json(...songByName);
      res.status(200).json(songByName);
    } else res.status(404).send(errorMessage.songs[0]);
  } else if (req.query.artist) {
    if (songByArtist.length) {
      songByArtist.length === 1 && res.status(200).json(...songByArtist);
      res.status(200).json(songByArtist);
    } else res.status(404).send(errorMessage.artists[0]);
  } else {
